...
def render_qdrant_query_debug() -> None:
    render_page_shell(
        "Qdrant Query Debug",
        "Run a query through the retrieval pipeline and inspect raw chunks — no LLM synthesis.",
    ...
    # --- Query inputs ---
    question = st.text_area("Query", placeholder="Enter your query here…", height=100)
    top_k = st.number_input("Top-K results", min_value=1, max_value=50, value=10, step=1)
    rerank = st.toggle("Rerank results", value=True, help="Apply semantic + temporal reranking. Disable to see raw Qdrant order.")
    ...
    for node in results:
        ...
        doc_name = meta.get("document_name") or meta.get("file_name") or "Unknown document"
        ...
        header = f"**#{rank}** — score `{score_str}`  ·  {doc_name}{page_str}{type_str}"
