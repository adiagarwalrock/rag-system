...
def parse(pdf_path: Path) -> tuple[str, float]:
    """Parse a PDF with Reducto and return (markdown, parse_time_seconds)."""
    ...
    if response.result.type == "url":
        ...
        chunks = requests.get(response.result.url).json().get("chunks", [])
    else:
...
if __name__ == "__main__":
    ...
    load_dotenv()  # load REDUCTO_API_KEY from .env file

    # For local testing
    pdf_path = Path("sample_docs/VICI-Investor-Presentation-Mar-26_.pdf")
    markdown, parse_time = parse(pdf_path)
    print(f"\nParsed Markdown:\n{markdown[:1000]}...")  # print first 1000 chars
    print(f"\nParse time: {parse_time:.2f} seconds")
